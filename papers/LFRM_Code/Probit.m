% % Code for implementing probit extension of LFRM as in Section 3.1 to
% % predict the probability of low birth weight
...
% % Weight : simulated birth weigth (in Kg)                           % %
...
% % set and predicted. To perform sequential predictions, change line 90 % %
% %                                                                      % %
% % Output files :                                                       % %
% % problbw.txt : contains estimated probability of low birth weight     % %
% % line 679 : plotting estimated trajectories                           % %
% % line 687 : computing predictive errors                               % %
% % line 711 : visualization of impact of covariates (boxplots)          % %
% % line 729 : ROC plot                                                  % %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fidalpha = fopen('alphaout.txt', 'wt');
fidgamma = fopen('gamma.txt', 'wt');
fidlbw = fopen('problbw.txt', 'wt');
fidlatent = fopen('latent.txt', 'wt');  
...
stringa11 = '%3.3f\t';
for i = 2:length(latent) - 1
    stringa11 = strcat(stringa11, ' ', type);
end
stringa10 = strcat(stringa11, ' ' , '%3.3f\r\n');
clear latent;
...
for g = 1:rep
    ...
    meanalpha = [norminv(.082, 0, 1)]'; b = .25;
    alpha = normrnd(meanalpha, sqrt(b));
    gamma = mvnrnd(zeros(1, k), eye(k));
    L = mvnrnd(repmat(0, n, 1), eye(1))';      % latent indicator variable for low birth weight
    ...
    for i = 1:nrun
      ...
      % --- Update alpha --- %
        margin = [];
        for j = 1:n
            margin(j) = L(1,j) - gamma*eta(j, :)';
        end
        alpha = normrnd((sum(margin) + meanalpha/b)/ (n+1/b), sqrt(1/(n+1/b)),1);
         
      % --- Update gamma --- %
        matricecov = eye(k) + eta'*eta;
        matricecov = inv((matricecov + matricecov) / 2);
        matricecov = (matricecov' + matricecov)/2;
        gamma = mvnrnd(matricecov*((eta')*(L(1,:) - repmat(alpha, n, 1)')'), matricecov);  
      ...
      % -- Update of eta -- % 
        for h = 1:n
            ...
            etavar = (Lambda')*(x')*(invinner)*x*Lambda + eye(k) + gamma'*gamma;
      ...
      % -- Update latent indicator for low birth weight -- %
        for j = 1:length(train)
          h = train(j);
         if resp(1, h) == 1
            L(1, h) = tnormrnd(alpha(1) + gamma(1,:)*eta(h,:)', 1, 0, Inf);
         else if resp(1,h) == 0
            L(1, h) = tnormrnd(alpha(1) + gamma(1,:)*eta(h,:)', 1, -Inf, 0);
             end
         end
        end
        for j = 1:length(test)
          h = test(j);
          L(1, h) = normrnd(alpha(1) + gamma(1,:)*eta(h,:)', 1);
        end
       ...
       % -- Save sampled values (after thinning) -- %
        if mod(i, thin) == 0 
           ...
           % -- probability of low birth weight -- %
             compproblbw = zeros(1, tstsize);
             for j = 1:tstsize
                 h = tst(j);
                 compproblbw(1,j) = normcdf(alpha(1) + gamma(1,:)*eta(h,:)', 0 , 1);
             end  
             fprintf(fidlbw, stringa9, compproblbw);
               
             Gammaone = zeros(50, 1);
             Gammaone(1:k, 1) = gamma(1, :);  
             fprintf(fidgamma, stringa10, Gammaone);
             clear Gammaone;
              
             fprintf(fidlatent, stringa11, L(1,:));
...
fclose(fidalpha); fclose(fidlbw); fclose(fidgamma); fclose(fidlatent);
...
for j = 1:length(tst);
    ...
    MSPE(:, sum(nitestobs(1: (index - 1))) + 1 : sum(nitestobs(1:index))) =  bsxfun(@minus,YtpredoutFINAL(1001:5000, sum(nitestobs(1: (index - 1))) + 1 : sum(nitestobs(1:index))),toplot(1: length(obs((obs > 30))))').^2;
    MAAB(:, sum(nitestobs(1: (index - 1))) + 1 : sum(nitestobs(1:index))) =  abs(bsxfun(@minus,YtpredoutFINAL(1001:5000, sum(nitestobs(1: (index - 1))) + 1 : sum(nitestobs(1:index))),toplot(1: length(obs((obs > 30))))'));
    MMAB(:, j) =  max(abs(bsxfun(@minus,YtpredoutFINAL(1001:5000, sum(nitestobs(1: (index - 1))) + 1 : sum(nitestobs(1:index))),toplot(1: length(obs((obs > 30))))')), [], 2);
end
MSPE = mean(mean(MSPE))
MAAB = mean(mean(MAAB))
...
load problbw.txt;
for j = 1:length(p)
    predY = mean(problbw(1001:5000, :)) > p(j);
    clas = lbw(test) == predY;
    ovmean(j) = mean(clas);                                 
    spec(j) = mean(clas(lbw(test) == 0 ));  % specificity
    sens(j) = mean(clas(lbw(test) == 1 ));  % sensitivity
end
plot((1-spec), sens)
title(['ROC plot'])
...
standev = std(problbw(1001:5000,:))./sqrt(4000);
